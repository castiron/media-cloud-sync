...
class MediaLibrary {
    ...
    public function get_attached_file($file, $attachment_id) {
        ...
        // If file doesn't exist locally and we're in image editor context, download it
        if ( $this->is_image_editor_context() ) {
            $downloaded = $wpmcsItem->moveToServer($attachment_id, 'full', 'media_library', true);
            if ( $downloaded && file_exists( $file ) ) {
                return $file;
            }
        }
    ...
    /**
     * Check if we're in an image editor context (crop, scale, rotate, etc.)
     */
    private function is_image_editor_context() {
        ...
        // WordPress image editor AJAX actions
        ...
        return in_array( $action, $image_editor_actions, true );
    }
